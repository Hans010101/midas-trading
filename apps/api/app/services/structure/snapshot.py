...
窗口口径(window 字段 · TTL 60d 约束的产品化,下游/LLM 不得冒充长期基线):
  ...
  - premium(基差)表仅存 7 天 → window="latest";FGI/dominance 全市场单点 → window="latest"。
...
async def build_structure_snapshot(client: AsyncClient, symbol: str) -> StructureSnapshot:
    """聚合 7 因子 → StructureSnapshot(纯读 · 不进缓存,缓存在 get_structure_snapshot)。"""
    ...
    funding_rows = await _safe(
        "funding_rate", select_funding_rates(client, sym, limit=_FUNDING_7D_LIMIT),
    )
    ...
    # ── ①② 大户账户/持仓多空比 + ③ taker(同一张 LSR 表 · 24h 窗)──────────────
    account = position = taker = None
    if lsr_rows:
        last = lsr_rows[-1]
    ...
    if funding_rows:
        ...
        funding = StructureFactor(
            value={
                ...
                "avg_7d": _round(fmean(rates), 6),
                "max_7d": _round(max(rates), 6),
                "min_7d": _round(min(rates), 6),
            },
            window="7d",
