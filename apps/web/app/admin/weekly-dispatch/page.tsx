 ...
 *   周日 21:00 定时发(无上传则提醒 admin)· 补救窗口(周日21:00~周一09:00)上传即发 · 也可手动立即发。
 */
...
export default function AdminWeeklyDispatchPage() {
  ...
  const uploadMut = useMutation({
    ...
    onSuccess: (res) => {
      setPreview(res)
      void qc.invalidateQueries({ queryKey: ['admin-weekly-dispatch'] })
    },
  })

  const sendMut = useMutation({
    mutationFn: (id: number) => sendWeeklyDispatchNow(token, id),
    onSuccess: () => void qc.invalidateQueries({ queryKey: ['admin-weekly-dispatch'] }),
  })
              ...
              <section className="mb-6 rounded-lg border border-gold/40 bg-gold/5 p-5 shadow-sm">
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {preview.auto_sent ? '✓ 补救窗口内已自动发送' : `状态:${STATUS_LABEL[preview.status] ?? preview.status}`}
                  </span>
                </div>
            ...
            <section className="rounded-lg border border-paper bg-cream shadow-sm">
                      ...
                      <tr key={it.id} className="border-b border-paper/60 last:border-0">
                        ...
                        <td className="px-4 py-3 text-right">
                          {it.status === 'uploaded' && (
                            <button
                              type="button"
                              onClick={() => sendMut.mutate(it.id)}
                              disabled={sendMut.isPending}
                              className="rounded border border-midas-red px-2.5 py-1 text-xs text-midas-red hover:bg-midas-red-glow/40 disabled:opacity-50"
                            >
                              立即发送
                            </button>
                          )}
              ...
              {sendMut.isError && (
                <p className="px-4 py-2 text-xs text-midas-red">
                  发送失败:
                  {sendMut.error instanceof AdminApiError ? sendMut.error.detail : '请重试'}
                </p>
              )}
            </section>
          </>
