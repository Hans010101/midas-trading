      ...
      <main className="flex-1">
        <div className="mx-auto max-w-[1600px] px-3 py-4 sm:px-6 sm:py-5">
          <div className="mb-3 flex justify-end">
            <DataTimestamp value={q.data?.as_of} locale={locale} />
          </div>
          {q.isPending && <LoadingNote className="py-10" />}
          {q.isError && (
            <EmptyState
              title={en ? 'Global market data is temporarily unavailable' : '暂时无法读取行情'}
              hint={en ? 'The data service will retry automatically' : '后端不可达 · 稍后自动重试'}
            />
          )}
          {q.isSuccess && groups.length === 0 && (
            <EmptyState
              title={en ? 'Global indicators are awaiting collection' : '全球指标数据待采集'}
              hint={en ? 'Snapshots are collected every 10 minutes' : '采集任务每 10 分钟写入快照'}
            />
          )}
          ...
          {groups.map((group) => (
            <section key={group.category} className="mb-6">
              <h2 className="mb-3 font-serif text-sm font-bold text-foreground">
                {en ? (GROUP_LABEL_EN[group.category] ?? group.label) : group.label}
              </h2>
