...
export function technicalDecisionFallback(snapshot: Readonly<{
  last_close: number
  ...
  if (snapshot.ma5 > snapshot.ma20) {
    score += 25
    findings.push(en
      ? 'the short-term average is above the medium-term average'
      : '短期均线位于中期均线上方')
  } else if (snapshot.ma5 < snapshot.ma20) {
    score -= 25
    findings.push(en
      ? 'the short-term average is below the medium-term average'
      : '短期均线位于中期均线下方')
  }
  if (snapshot.ma20 > snapshot.ma60) {
    score += 20
    findings.push(en ? 'the medium-term trend remains upward' : '中期趋势保持向上')
  } else if (snapshot.ma20 < snapshot.ma60) {
    score -= 20
    findings.push(en ? 'the medium-term trend remains downward' : '中期趋势保持向下')
  }
  ...
  const momentum =
    snapshot.change_20_bars_pct >= 0
      ? en
        ? `price rose ${snapshot.change_20_bars_pct.toFixed(2)}% over 20 bars`
        : `近 20 根 K 线上涨 ${snapshot.change_20_bars_pct.toFixed(2)}%`
      : en
        ? `price fell ${Math.abs(snapshot.change_20_bars_pct).toFixed(2)}% over 20 bars`
        : `近 20 根 K 线下跌 ${Math.abs(snapshot.change_20_bars_pct).toFixed(2)}%`
  const rationale = en
    ? `${findings.join('; ') || 'moving averages show no clear direction'}; ${momentum}; RSI(14) is ${snapshot.rsi14.toFixed(1)}.`
    : `${findings.join('，') || '均线结构暂未形成明确方向'}；${momentum}，RSI(14) 为 ${snapshot.rsi14.toFixed(1)}。`
  return {
    score,
    confidence: bounded(
    ...
    plan_note:
      Math.abs(score) < 20
        ? en
          ? 'Wait for price to break the recent range before confirming direction'
          : '等待价格突破近期区间后再确认方向'
        : en
          ? `Use the recent ${score > 0 ? 'low' : 'high'} as the structure invalidation reference`
...
async function decisionCard(
  ...
  if (
    ...
    throw new HttpError(400, en ? 'Invalid AI decision-card parameters' : 'AI 决策卡参数格式无效')
  }
  if (!bearerToken(request)) {
  ...
  if (klines.items.length < 30) {
    throw new HttpError(422, en ? 'Insufficient candlestick data' : 'K 线数据不足')
  }
  ...
  try {
    ai = await invokeAi(env, {
      system: en
        ? 'You are a professional market technician. Use only the supplied structured data and return concise JSON in English. Do not add disclaimers, risk warnings, marketing copy, or a fixed closing line.'
        : '你是专业市场技术分析师。只依据给定结构化行情输出 JSON；语言简练，不添加免责声明、风险提示、营销话术或固定结尾。',
      prompt: en
        ? `${JSON.stringify(snapshot)}
Return strict JSON:
{"score":-100 to 100 integer,"confidence":0 to 1,"rationale":"2 to 4 sentences of technical structure analysis","key_levels":[support,resistance],"plan_note":"one concise execution-plan sentence"}
...
不得输出 Markdown。`,
  ...
  const actionable = direction === 'hold'
    ...
    : {
        direction,
        actionable: true,
        basis: en ? `Technical score ${score}` : `技术评分 ${score}`,
        size_note: en ? 'Use account risk budget' : '按账户风险预算执行',
        hint: planNote,
        disclaimer: '',
      }
  const body = {
