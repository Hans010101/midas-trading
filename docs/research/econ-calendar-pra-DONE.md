...
## 追加 · UI 微调三处(2026-07-10 · PR #187 · 普通 PR CI 兜底)

Hans 拍板三条打磨(纯前端 · 零数据零筛选逻辑):
1. 删页内「财经日历」大标题(导航 Tab 已有同名入口)
2. 说明句「官方宏观事件日程 · 时间均为北京时间 · 重大事件公布前后市场波动可能放大,
   供参考」挪到筛选行下方——★红线文案一字未改只挪位,完整性锁复跑仍绿(18 passed)
3. 分组标题去数量计数(保留 今天/本周/未来);保鲜标注收进筛选行右侧

三件套:Actions run 29093533362 绿 · `midas-web Recreated`(仅 web,符合纯前端改动)·
真机 curl:`<h1>` 大标题 0 处 / 说明句 1 处(逐字)/ 免责 1 处。

## 追加 · 日历页两处UI精简(2026-07-11 · PR #191/#192 · 普通PR CI兜底)

Hans 两条打磨(纯前端·零碰后端 importance/决策卡门槛/markets 双锁):
1. **#191 删「未来」分组冗余标题**:列表按时间排序+每条带日期,「未来」catch-all 标题无
   信息价值 → 删;「今天/本周」保留(紧迫度信号有值)。未来事件仍 chronological 流在下方。
2. **#192 删「全部重要度/仅重要★2+」筛选 tab**:重要度筛选无意义(加密全★2+筛了没变·
   欧洲日韩全★1筛了变空需补丁)→ 删两按钮 + majorOnly 逻辑 + 拆★2+空态精确提示补丁
   (触发前提没了)。★保留每条事件星级标注(★渲染)——删的是「按星级筛选」不是「显示星级」。
   各地区现正常显示全部事件。

★红线守住:后端 importance 赋值 / 决策卡 min_importance≥2 / markets 双锁一律没动;
CI 红线锁全绿未误伤(免责/方向词/零LLM/字段集/never-inject 变异)。

三件套:#191 run 29144413258 / #192 run 29144666125 均 Actions 绿 · **仅 web Recreated**
(api/worker 未触碰,符合纯前端)· 真机 curl:重要度 tab 0 处、免责+波动句各 1 处原样。
