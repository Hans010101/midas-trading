...
## 状态
**Draft**(2026-05-31 · 待产品负责人审 + 拍板 §待拍板细节)· 本 ADR 只出设计,不含实现代码。审过 + 拍板 → 按 §8 分单元开工(MVP = 前 3 单元)。
...
- ★ **口径说明(待拍板细节①)**:这里用**「反弹确认式」**(穿越回归 30/70),而非「进入超卖即买」(下穿 30)。理由:① 「反弹/回落」语义 = 极值后**回归**;② 穿越式离散,避免 RSI 持续 < 30 时连续标点的噪音。**如产品要「进入超卖即买」(下穿 30 买 / 上穿 70 卖),请拍板改口径**(影响信号点位置)。
...
- ★ **判定基准(待拍板细节②)**:用**收盘价 `C` 穿越**(推荐:稳、与 K 线收盘对齐);备选用当根最低/最高价 `touch`(更敏感、信号更多)。

> **信号 kind 语义(待拍板细节③)**:形态A 信号统一是抽象**「买/卖」(看涨/看跌)**,**不区分现货/合约**(因为形态A 不下单)。用户看完去第一层下单时,由第一层既有映射处理(现货 buy/sell、perp open_long/open_short,见 [actionable.py:36-62](../../apps/api/app/services/ai/actionable.py))。形态A 不重复这层映射。
...
- 挂在现有 [analysis.py](../../apps/api/app/api/v1/analysis.py)(已有本地 `DbDep` 等),**新增只读 GET 端点**(无鉴权,与 decision-card / ai-accuracy 一致):
  ...
  - **(待拍板细节④)** 是否合并成一个端点(不传 strategy 返回推荐 + 推荐策略信号;传 strategy 返回该策略信号)。推荐:**先拆两个端点**(职责清晰,前端按需调)。
- ★ **只读**:只 `select_kline` 读 CH 已采历史 K 线,**不打实时上游**;K 线不足时回源逻辑沿用 decision-card 既有模式(或直接返回「数据不足」让前端兜底)。
- **crypto instrument(待拍板细节⑦)**:crypto 用 `spot` 还是 `perp` K 线扫描?推荐:**跟随当前详情页**(crypto-preview 是 perp → 传 `instrument=perp`),与缠论一致。
...
| # | 议题 | 推荐 |
|---|---|---|
| ① | **RSI 信号口径** | **反弹确认式**(上穿 30 买 / 下穿 70 卖,§2 策略2)· 备选「进入超卖即买」(下穿 30 买) |
| ② | **布林触轨判定基准** | **收盘价 `C` 穿越**(稳、与收盘对齐)· 备选当根最高/最低价触及(更敏感) |
| ③ | **信号 kind 语义** | **统一抽象「买/卖」**(不区分现货/合约,看完走第一层)· 不在形态A 重复 spot/perp 映射 |
| ④ | **API 形态** | **先拆两个端点**(`/strategy-signals` + `/strategy-recommend`)· 备选合并 |
| ⑤ | **参数是否可调** | **MVP 固定默认**(MA5×20、RSI14·30/70、BOLL20·2σ)· 用户可调留后续 |
| ⑥ | **信号回看窗口** | K 线上标**全部可见历史信号点**(随图缩放)· 备选只标最近 N 个 |
| ⑦ | **crypto 用 spot 还是 perp K 线** | **跟随详情页**(crypto-preview=perp → instrument=perp,与缠论一致) |
...
> 本 ADR 为 Draft,待产品负责人审(★展示型边界 + 排除自动交易/沙箱 + 3 套主图覆盖 + 零回归)+ 拍板上表 7 个细节 → 转 Accepted,按 §8 分单元开工(MVP = 前 3 单元)。本轮只出设计,不含实现代码。
