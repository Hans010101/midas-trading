...
- **`margin_mode` 加 `cross` 值**(DP-5):
  ...
  - **倾向选项②**(更可逆、迁移更干净;代价是丢 DB 层 enum 约束,改由应用层 + schema 校验)。DP-5 定。
...
> 我识别出的「需要产品方决定的算法细节」如下。建议你把这些整理成选择题。

| # | 决策 | 选项 |
|---|---|---|
| **DP-1** | **是否允许同时持有逐仓 + 全仓** | ① per-symbol 模式(不同 symbol 可不同模式,同 symbol 同时只一种 · 仿 Binance · 推荐)② 账户级单一模式(整个 crypto 账户要么全逐仓要么全全仓 · 更简) |
| **DP-2** | **全仓强平方式** | ① 全账户一次性全平(教学最简 · 推荐起步)② 分级部分强平(贴近真实 · 复杂) |
| **DP-3** | **全仓强平顺序**(仅当 DP-2 选②) | ① 亏损最大优先 ② 维持保证金占用最大优先 ③ 名义额最大优先 |
| **DP-4** | **逐仓 ⇄ 全仓资金互转 / 调整保证金** | ① 本期不做(开仓后保证金固定 · 最简)② 做(支持加/减保证金 · 真实但复杂) |
| **DP-5** | **margin_mode 存储** | ① Enum `ADD VALUE 'cross'`(不可逆)② 改 `String(16)`(可逆 · 推荐) |
| **DP-6** | **全仓维持保证金率** | ① 沿用逐仓 0.005 全局 ② 分档(按名义额阶梯 · 真实但需档位表) |
| **DP-7** | **同 symbol 是否允许逐仓 + 全仓并存** | ①否(同 symbol 同时只一种模式 · 仿真实 · 推荐)②是(需拆约束 · 不推荐) |
| **DP-8** | **全仓穿仓(equity<0)** | ① 地板到 0,平台虚拟承担(同逐仓 · 推荐)② 记负债 |
| **DP-9** | **全仓某仓缺 mark 时** | ① 本轮跳过该账户强平判定(保守 · 推荐)② 用该仓上次已知 mark |
| **DP-10** | **MC 分期是否按 §8** | 采纳 / 调整 |

---

## 10. 备注
本文只是方案,不含实现代码、不改运行代码。产品方就 §9 决策点拍板后,按 §8 分期、每期 feature 分支 +
严格审(迁移逐行审 · 引擎核心碰不得 · 改动严格隔离)落地。
