...
- 状态:**实施细案(待审)** · 2026-05-30 · 关联:ADR 0034(港股接入)+ `scripts/hk-phase0/`(阶段零探测)
...
## 需产品负责人拍板的点
1. **每手股数策展清单**:现在定首批名单 + lot 种子值(§4.2 列了 ~18 只待核),还是阶段三前再定?
   (建议现在把**结构 + 种子值**定下来,真实 lot 值阶段三上线前核 HKEX。)
2. **首批港股标的池**:接受 §4.2 这 ~18 只核心(蓝筹 + 科技 + 中概回港)做首批,阶段四扩到 ~120?
3. **分钟级 K 线**:若 akshare 港股不支持分钟级,阶段一**只做日 / 周线**(对齐首批以日线为主)可接受?
4. **CH 迁移执行**:确认走 §1.2「Hans 手动 clickhouse-client ALTER(带外)+ 低峰停 worker」机制;P1-1 我产出可执行迁移脚本(像 probe 但执行 ALTER + 护栏 + 回滚)给 Hans。
