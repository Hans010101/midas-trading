...
- 状态:**Accepted**(2026-05-30 审过 · 6 决策点已拍板,见文末)· 阶段零实测前置未过前不进写代码阶段
...
## 决策点(已拍板 · 2026-05-30)
1. **数据源主源** → **阶段零实测 0700.HK 后定**(零-B 输出 akshare vs yfinance 质量/延迟/字段对比再选主源)。
2. **每手股数兜底** → **认可**:策展配置覆盖热门港股;**未覆盖标的暂不可下单**(给提示)。
3. **HKD 虚拟费率** → **象征性**(简单、非拟真):印花税 + 佣金给个象征小值,不追求真实档位。
4. **港股标的范围** → **策展热门池**(腾讯 00700 / 阿里 09988 / 美团 03690 等主流,**对齐美股精选规模 ~120 只**);全市场后期扩。
5. **CH 迁移窗口** → **低峰执行 + 暂停 worker 保险**(阶段一迁移时短暂停采集 worker,迁移完恢复)。
6. **板块榜单** → 放**阶段四(可选)**,首批不做。

> 阶段零三项实测(零-A CH 迁移探测 / 零-B 0700.HK 数据实测 / 零-C 每手字段)由 Hans 在服务器跑只读探测脚本
> (`scripts/hk-phase0/`)· 实测结果回来定主源(决策 1)+ 迁移可行性 + 每手字段有无,**实测过才进阶段一写代码**。
