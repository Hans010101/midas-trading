...
**Approved**(2026-05-28)· 7 个决策点(DP-G5-1..7)产品方已全部拍板(见 §8 终结论)·
按 §9 在 `feat/m1g-g5-impl` 实施中。0025 §9 G5 的详细设计。
...
## 8. 决策点(最终结论 · 产品方已拍板 2026-05-28)

| # | 决策 | 最终结论 |
|---|---|---|
| **DP-G5-1** | 后台预设默认值 | **采纳**:杠杆 3 · 永续 100 USDT · A股 10000 CNY · 美股 1000 USD(与 G4 安全常量一致 · 零回归)。 |
| **DP-G5-2** | 默认推荐规则集 | **采纳 9 条完整版**(§4.2):Tier1 全局 3 条(crypto 恐贪 <20/>80 + A股上涨家数 <30%)+ Tier2 per-symbol 6 条(NVDA/600519 RSI 70/30,BTC/USDT RSI 75/25)· 后端常量 `RECOMMENDED_ALERT_RULES`(单一事实源 · bot + 网页共用)。 |
| **DP-G5-3** | 推荐规则种入方式 | **采纳 B**:用户主动「一键应用推荐规则」· `POST /alert-rules/apply-recommended`(跳过已存在 / 超 20 上限)· 不加用户表标志位、不加额外迁移。 |
| **DP-G5-4** | bot 头像 / 简介 | **采纳**:印章 logo(M1-C 朱文方印)+ 简介「点金 Midas · AI 多市场分析终端(A股/美股/加密)· 全程虚拟资金,仅供参考不构成投资建议」· @BotFather 配置由产品方做,G5 代码无需为此写。 |
| **DP-G5-5** | 命令菜单 | **采纳代码侧 `setMyCommands`**:在 `register_webhook_if_configured()` 启动期顺带调一次,命令菜单随代码同步、免手动 @BotFather 维护。 |
| **DP-G5-6** | bot 内告警配置深度 | **采纳 查看 + 启停 + 一键推荐**;不在 bot 里做全量新建规则(留网页)· 复用 G3 会话态 + inline 键盘。 |
| **DP-G5-7** | 后台预设 UI 落位 | **采纳 `/settings` 同页**(与通知 / 配色 section 同页)· 新增 `BotOrderPresetSection`。 |
