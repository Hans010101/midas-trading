...
**DP1 · 核心策略组合**:
1. ① **A + C + E**(P0 推荐 · 根治 + 直观开关 + 默认轻调)
2. ② A + C(只做根治 + 安静时段,默认阈值不动)
3. ③ 只做 B + C(不做 A 重构,只把默认冷却拉到 24h + 加安静时段)
4. ④ 全做(A + B + C + D + E + F,大投入)

**DP2 · A(edge-triggered)的状态存储**:
1. ① Redis 持久化(无 TTL)· 轻 · 重启会丢状态(首次扫再推一次,可接受)
2. ② 新 `alert_rule_state` 表 · 可逆迁移 · 更可靠

**DP3 · 默认冷却(若做 B/F)**:
1. ① **F · 分层默认**(技术 24h / 全局 6h / 快变 30min / chan/crypto_deriv 1h · 推荐)
2. ② B · 全规则统一改为 24h
3. ③ 不动默认(只靠 A 解决)

**DP4 · 安静时段默认值**:
1. ① 默认禁用(用户主动开 · 推荐 · 不打扰行为不变)
2. ② 默认 22:00–08:00 / `Asia/Shanghai`(主动降噪)
3. ③ 默认禁用 + 注册流程引导一次

**DP5 · 安静时段时区**:
1. ① 用户档存 IANA 时区(注册时前端 `Intl.DateTimeFormat().resolvedOptions().timeZone` 写入 · 推荐)
2. ② 全局固定 `Asia/Shanghai`(单地区运营 · 最简)
3. ③ 用户在设置页手选(可空 · 缺省 `Asia/Shanghai`)

**DP6 · 推荐规则默认阈值(E)**:
1. ① **轻收紧**(fear_greed 15/85, RSI 25/75 + crypto 20/80, breadth 25 · 推荐)
2. ② 不调
3. ③ 重收紧(fear_greed 10/90, RSI 20/80 全 symbol)

**DP7 · 现有用户的 `cooldown_sec` 是否回填默认**:
1. ① 不回填(尊重个性化 · 推荐)· 老用户保留 300s
2. ② 全用户回填到新默认(更激进的全局降噪)
3. ③ 只回填"没改过的"(== 旧默认 300s 的)到新默认

**DP8 · 同类聚合 / digest(D)**:
1. ① **本期不做**,P2 跟进(推荐)
2. ② 本期一起做(投入大但体验最佳)

**DP9 · 老 ±5%(price_alerts)是否纳入降噪**:
1. ① 安静时段覆盖它(dispatcher 层拦截 · 推荐)
2. ② 完全不动
3. ③ 一并迁到新引擎(把"涨跌幅 ±5%"做成内置规则 · 工作量较大)

**DP10 · 紧急告警豁免**(若做 D 或 C):
1. ① **本期不分级**,所有规则一视同仁(推荐 · 简单)
2. ② 分"紧急 / 一般",紧急豁免 quiet(需在 indicator 或 rule 上加 priority 字段)
