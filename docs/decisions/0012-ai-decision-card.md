...
## 状态
**Approved + Shipped** (2026-05-21) · M1 二波真实 DeepSeek 调用验收通过(详见末尾 § M1 二波验收记录) · M2 升级到 4 Agent 时再修订
...
## M1 二波验收记录(2026-05-21)

产品负责人 2026-05-21 提供 DEEPSEEK_API_KEY,执行三市场端到端真实调用 ·
验收通过 · M1 第二波正式收口。

### 实测真实 DeepSeek 输出(BTC / NVDA / 600519 各 1 次)

| 标的 | composite | 置信度 | 关键位 | tokens(prompt / completion / total) |
|---|---|---|---|---|
| BTC/USDT | -30(弱空)| 70% | 76000 / 79411 | 481 / 109 / **590** |
| NVDA | -30(弱空)| 60% | 213.44 / 225.64 | 427 / 122 / **549** |
| 600519 | -30(弱空)| 70% | 1307.6 / 1383.2 | 445 / 137 / **582** |

三段真实 narrative(完整保留)便于将来对比 prompt 迭代效果:

> **BTC/USDT:** 当前价格位于MA5和MA20之下,MA5下穿MA20,MACD柱线为负且DIF在DEA下方,显示空头动能。RSI中性偏弱。布林带中轨向下,价格接近下轨。缠论出现三卖信号,最后一笔方向向下,短期偏空。

> **NVDA:** 价格位于MA5下方,短期趋势向下;MACD柱状线为正但DIF与DEA接近,上行动能减弱;RSI处于中性偏强区域但已从高位回落;布林带中轨向上但价格已跌破中轨,有向中轨回归可能。缠论最后一笔向上,但近期价格回调,需关注是否形成新的买卖点。

> **600519:** 价格位于所有均线下方,均线空头排列;MACD双线在零轴下方,柱线负值扩大;RSI低于30,进入超卖区域;布林带中轨下压,价格接近下轨。缠论显示一买和二买已出现,但二买位置较高,且当前价格低于二买点,反弹力度不足。整体空头趋势未改,但超卖可能引发短期反弹。

### 产品负责人质量评估

- ✅ 引用了具体的 MA/MACD/RSI/BOLL 数据,不是套话
- ✅ 缠论解读跟 K 线结构对得上(尤其 600519「二买位置高、当前价低于二买、反弹力度不足」—— 真在读结构)
- ✅ 用词克制,无祈使句,Validator 都没触发(LLM 自己也守规矩)
- ✅ 三个标的有明显区分度(纯空 / 拉锯 / 超卖反弹),不是模板套话
- ✅ 「3 个都弱空」**不是问题** —— 当天 BTC/NVDA/600519 确实都在回调,数据真实如此;且评分是 -30「弱」不是「强空」,三段论据完全不同,说明是独立分析。

**判定:** AI 真在分析,质量达到「能用」标准。

### 实测成本(对比 0012 § 成本估算章节)

| 指标 | 0012 估算 | 实测 | 差异 |
|---|---|---|---|
| 单次成本(全冷)| $0.000546 ≈ ¥0.0039 | ¥0.000676 - ¥0.000725 ≈ **¥0.0007** | **实际更便宜 ~5.5 倍** |
| 100 用户月费(含应用层缓存)| ¥9.4 | **¥1.7** | 实际更便宜 ~5.5 倍 |
| 缓存命中提速 | 估算 ~3x | 实测 35x(3.5s → 0.1s)| 远超预期 |

**实测成本结构:** prompt:completion ≈ 4:1(单次决策卡 input 重 / output 轻 ·
跟 0012 估算 2500:700 ≈ 3.6:1 一致)。

**月度预算结论:** ¥200/月硬上限**远未触顶** · 实测水平下可支持 ~12,000 用户 · M2 升级到 4 Agent 后预算仍宽裕。

### narrative 策略最终决定

**M1 二波 narrative 保持「克制、纯描述」风格,不加「观察提示」类内容。**

理由:
- 加「观察提示」(eg.「关注 X 价位是否破位」「若回踩不破可能反弹」)有合规风险 · 即使措辞中性,也可能被解读为隐含建议
- M1 二波目标是先让用户**看到 AI 在认真分析**,质量达标即可,不追求所谓「实用」
- 等 M2 真实用户反馈再议是否扩充叙事维度

不在本 ADR 范围(留 M2+ 决策):
- prompt 是否加入「场景化提示」(eg. 区分突破 / 回调 / 横盘语境)
- 是否给不同信心度的 narrative 配不同语气
- 是否加入「与历史相似走势对比」(需要向量数据库)

### 代码 mock 边界审计(M1 二波收口检查)

产品负责人要求确认「mock 边界已全部转为真实,没有 mock-only 分支影响生产路径」:

| 位置 | 类型 | 是否影响生产 |
|---|---|---|
| `app/services/ai/llm.py` 真实 LiteLLM 分支前的 TODO 注释 | 文档 | 已删除 TODO 字样 · 改为生产主路径注释 |
| `llm.py:is_mock_mode()` | 兜底逻辑 | ✅ **保留**(产品负责人指令):key 缺失时 UI 仍可用 |
| `llm.py:_mock_invoke()` | 兜底实现 | ✅ **保留**:只在 `is_mock_mode()` 为真时调用 · key 在生产时不会触发 |
| `llm_mock_mode=True` env | 测试用 | ✅ 保留:`tests/test_ai_decision.py` 走 mock · CI 不烧 token |
| `DecisionCardResponse.llm_mode` | API 字段 | 生产返回 `'real'` · key 缺失时返回 `'mock'` · 前端按字段决定是否显示「mock(待填 KEY)」徽章 |
| 前端 `card.llm_mode === 'mock'` 判断 | UI 兜底 | ✅ 保留:key 缺失场景的视觉提示 |

**审计结论:无 mock-only 分支会影响有 key 时的生产路径。**

### Git tags 落锤

- `checkpoint-y` · AI 后端(mock 阶段)· commit `72b3f0f`
- `checkpoint-z` · AI 前端 UI · commit `095c1f5`
- `checkpoint-y-real` 标记真实调用验收点 · 本节配套打 tag(见本 commit)

### M2+ 待办(从本 ADR 派生)

1. **Aggregator 重激活**(M2 加基本面 Agent 后)
2. **prompt 场景化**(突破 / 回调 / 横盘 系统提示分支)
